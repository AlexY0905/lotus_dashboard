...
func newLocalListener() net.Listener {
	service := "127.0.0.1:12666"
	tcpAddr, err := net.ResolveTCPAddr("tcp", service)
	//service_tcp6 := "[::1]:12666"
	//tcpAddr_tcp6, err := net.ResolveTCPAddr("tcp6", service_tcp6)
	if err != nil {
		return nil
	}
	if serveFlag != "" {
	...
	/*if err != nil {
		if l, err = net.ListenTCP("tcp6", tcpAddr_tcp6); err != nil {
...
func (s *Server) goServe() {
	...
	s.wg.Add(1)
	go func() {
		/*s.Config.SetKeepAlivesEnabled(true)
		for {
			fmt.Println(s.Listener.Addr())
			...
			s.wg.Add(1)
			go doServerContent(s, conn)
		}*/
		s.Config.SetKeepAlivesEnabled(true)

		err := s.Config.Serve(s.Listener)
		s.wg.Add(1)
...
func doServerContent(s *Server, conn net.Conn) {
	defer s.wg.Done()
	defer conn.Close()
